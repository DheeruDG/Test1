...
//    [NSNotificationCenter.defaultCenter addObserver:self
//                                           selector:@selector(changeitstatus)
...
//                                             object:nil];
    ...
    self.view.backgroundColor=kColorHeader;
}
...
-(void)changeitstatus
{
//    if ([Razauser SharedInstance].callmodeofstatus.length) {
...
- (void)updateUI:(BOOL)inCall {
 ...
 if (lockunlock.length)
   _outcallView.hidden=YES;

    
    [self performSelector:@selector(updateuimethod) withObject:self afterDelay:1.0 ];
...
- (void)callSecurityUpdate {
	...
	if (list == NULL) {
		...
		_callSecurityButton.hidden = YES;
		while (list != NULL) {
		...
		[_callSecurityButton setImage:[UIImage imageNamed:@"back_default.png"] forState:UIControlStateNormal];
	}
}
...
- (void)callQualityUpdate {
	...
	if (call != NULL) {
		...
		if (![accessibilityValue isEqualToString:_callQualityButton.accessibilityValue]) {
			...
			_callQualityButton.hidden = (quality == -1.f);//NO; //
//			UIImage *image =
//				(quality == -1.f)
...
//					: [UIImage imageNamed:[NSString stringWithFormat:@"call_quality_indicator_%d.png", quality]];
			[_callQualityButton setImage:[UIImage imageNamed:@"back_default.png"] forState:UIControlStateNormal];
		}
	}
    [self performSelector:@selector(updateuimethod) withObject:self afterDelay:1.0 ];
    //[self updateuimethod];
...
if (!call) {
    UICompositeViewDescription *vv=PhoneMainView.instance.currentView;
    if ([vv.name isEqualToString:@"CallView"])
...
//        UICompositeViewDescription *vv=PhoneMainView.instance.currentView;
//        NSLog(@"%@",vv.name);
//        if (![vv.name isEqualToString:@"CallView"]||![vv.name isEqualToString:@"UIConfirmationDialog"] CallOutgoingView.xib)
...
{
    UICompositeViewDescription *vv=PhoneMainView.instance.currentView;
    if ([vv.name isEqualToString:@"CallView"]||[vv.name isEqualToString:@"CallIncomingView"]||[vv.name isEqualToString:@"CallOutgoingView"])
        _callQualityButton.hidden=YES;
    else
        _callQualityButton.hidden=NO;
