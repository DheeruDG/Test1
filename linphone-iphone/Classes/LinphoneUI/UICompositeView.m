...
- (id)init:(Class)content
    ...
    tabselected=NSStringFromClass(content);
...
- (UIInterfaceOrientation)currentOrientation {
    return UIInterfaceOrientationPortrait;//currentOrientation;
}
...
//    if ([description.name isEqualToString:@"HistoryListView"])
//    self.mainView.frame = CGRectMake(mainFrame.origin.x,66, mainFrame.size.width, mainFrame.size.height);//CGRectMake(0, 62, 320, 418)//mainFrame;
//    else
        self.mainView.frame = mainFrame;//CGRectMake(mainFrame.origin.x,66, mainFrame.size.width, mainFrame.size.height);
	...
	// 2. tab bar
    NSLog(@"%@",tabselected);
    if ([description.name isEqualToString:@"DialerView"])
        self.tabBarView.frame =tabFrame;// self.tabBarView.frame =CGRectMake(0, 70, SCREENWIDTH, 66) ;
    else
...
- (void)update:(UICompositeViewDescription *)description
     ...
     statusBar:(NSNumber *)statusBar
      sideMenu:(NSNumber *)sideMenu
    fullscreen:(NSNumber *)fullscreen andmystring:(NSString*)dialpad {
    ...
    // 2. tab bar
    NSLog(@"%@",tabselected);
    if ([description.name isEqualToString:@"DialerView"]||(dialpad.length))
       // self.tabBarView.frame =CGRectMake(0, 70, SCREENWIDTH, 66) ;
        self.tabBarView.frame =tabFrame;
    else
        self.tabBarView.frame =tabFrame;
    //tabFrame;//
...
- (void)hideSideMenumain:(BOOL)hidden {
    [self update:nil tabBar:nil statusBar:nil sideMenu:[NSNumber numberWithBool:!hidden] fullscreen:nil andmystring:@"dialpad"];
}
