...
- (void)accessoryForCell:(UITableViewCell *)cell atPath:(NSIndexPath *)indexPath {
	...
	if ([self isEditing]) {
		...
		if ([_selectedItems containsObject:indexPath]) {
			accessoryImage = [UIImage imageNamed:@"checkbox_checked.png"];
			checkBoxButton.accessibilityValue = NSLocalizedString(@"Selected", nil);
		} else {
			accessoryImage = [UIImage imageNamed:@"checkbox_unchecked.png"];
			checkBoxButton.accessibilityValue = NSLocalizedString(@"Deselected", nil);
		}
		[checkBoxButton setImage:accessoryImage forState:UIControlStateNormal];
        ...
        UIImageView* accessoryImageView = [[UIImageView alloc] initWithFrame:
                                           CGRectMake(0, 0, accessoryImage.size.width , accessoryImage.size.height)];
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated {
	...
	_editButton.hidden=_backButton.hidden = editing;
