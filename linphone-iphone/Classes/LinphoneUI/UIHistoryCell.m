...
- (void)update {
	...
	if (linphone_call_log_get_dir(callLog) == LinphoneCallIncoming) {
		if (linphone_call_log_get_status(callLog) != LinphoneCallMissed) {
            displayNameLabel.textColor=OxfordBlueColor;
            image = [UIImage imageNamed:@"Incoming Call.png"];
        } else {
            displayNameLabel.textColor=[UIColor redColor];
            image = [UIImage imageNamed:@"Missed Call.png"];
        }
        addr = linphone_call_log_get_from_address(callLog);
    } else {//Outgoing Call
        if (linphone_call_log_get_status(callLog) != LinphoneCallMissed) {
            displayNameLabel.textColor=OxfordBlueColor;
            image = [UIImage imageNamed:@"Outgoing Call.png"];
        } else {
            displayNameLabel.textColor=[UIColor redColor];
            image = [UIImage imageNamed:@"Missed Call.png"];
        }
...
//        image = [UIImage imageNamed:@"Outgoing Call.png"];
	...
	[ContactDisplay setDisplayNameLabelRaza:displayNameLabel forAddress:addr];
    ...
    if ([razauser containsObject:str])
        [[Razauser SharedInstance] setRazaimageforimage:_avatarImage andstringname:str andthumbornot:RAZA_PROFILETHUMB];
    else
