...
- (void)setChatMessage:(LinphoneChatMessage *)amessage and:(MSList*)mm {
	...
	[super setChatMessage:amessage and:mm];
}
...
- (void)update {
		...
		if ([localImage isEqualToString:@"saving..."]) {
			...
			if (_messageImageView.image == nil) {
                ...
                [[PHImageManager defaultManager] requestImageForAsset:asset
                                                           targetSize:CGSizeMake(200, 200) // TODO: your target size
                                                          contentMode:PHImageContentModeAspectFill
                                                              options:options
                                                        resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info)
                 {
                     // Do whatever you want to the result
                     if (achat == self.message){
                         [_messageImageView setImage:result];
                ...
                [LinphoneManager.instance.photoLibrary assetForURL:imageUrl
                    resultBlock:^(ALAsset *asset) {
                      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, (unsigned long)NULL),
                                     ^(void) {
                                       if (achat == self.message && asset!=nil) { // Avoid glitch and scrolling
	...
	if (!fullScreenImage) {
        _messageImageView.alpha=0.3f;
    }else{
        _messageImageView.alpha=1.0f;
    }
...
- (IBAction)onImageClick:(id)event {
	...
	//	[self onResendClick:event];
		...
		if (![_messageImageView isLoading]) {
            ...
            UIImage *fullScreen = _messageImageView.image;//[UIImage imageWithCGImage:fullScreenRef];
...
-(void)showhideimage
{
    if ((_downloadButton.hidden==YES)&&(_cancelButton.hidden==YES) ) {
        _imageSubView.hidden=YES;
    }
    else
        _imageSubView.hidden=NO;
}
@end
