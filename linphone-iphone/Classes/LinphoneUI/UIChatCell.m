...
- (void)updateUnreadBadge {
    ...
    NSString *contactno=  [NSString stringWithUTF8String:uri];
            NSDictionary *dd=  [[Razauser SharedInstance]getPushrazaCounter];
            if ([[dd objectForKey:contactno] integerValue])
                count=count+[[dd objectForKey:contactno] intValue];
    }
...
-(void)setrazauser:(const LinphoneAddress *)imageadd
{
//    BOOL contactfound;
//    NSString *userphone;
...
//    if (contactfound) {
    ...
    NSString *phone=[[Razauser SharedInstance]getusername:str];
        NSURL *urluser=[NSURL URLWithString:[NSString stringWithFormat:@"%@%@.png",RAZA_PROFILE,phone]];
        [[Razauser SharedInstance] downloadImageWithURL:urluser completionBlock:^(BOOL succeeded, UIImage *image) {
            if (image)
            {
                ...
                [_avatarImage setImage:[FastAddressBook imageForAddress:imageadd thumbnail:YES] bordered:NO withRoundedRadius:YES];
