...
- (void)setAddress:(NSString *)address {
	...
	if (contact) {
        if (contact.phoneNumbers.count<=_indexPath.row) {
            _linphoneImage.hidden=YES;
        }else{
            NSString *numberKey = [RazaHelper getValidPhoneNumberWithString:[contact.phoneNumbers objectAtIndex:_indexPath.row]];
            
            if ([[[Razauser SharedInstance]getRazauser] containsObject:numberKey]) {
                _linphoneImage.hidden=NO;
            }else
                _linphoneImage.hidden=YES;
        }
...
//	if (addr)
//		linphone_address_destroy(addr);
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"makeacallviaRazaoutcontactdetailparam" object:_addressLabel.text];
      ContactDetailsView *view = VIEW(ContactDetailsView);
    //makeacallparamContactdetail
    [view makeacallparamContactdetail:_addressLabel.text];
