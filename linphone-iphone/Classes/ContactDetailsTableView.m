...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_contact.phoneNumbers.count==1) {
        _phone=_contact.phoneNumbers[0];
    }
    ...
    if (indexPath.section == ContactSections_FirstName) {
        ...
        cell.editTextfield.placeholder=@"Phone number";
        value = _contact.phoneNumbers[indexPath.row];
        //cell.addressLabel.textColor=[UIColor colorWithRed:(100.0f/255.0f) green:(100.0f/255.0f) blue:(100.0f/255.0f) alpha:1];
        //        if (indexPath.row==0&&!self.tableView.isEditing) {
        //            cell.addressLabel.frame=CGRectMake(cell.addressLabel.frame.origin.x, 38, cell.addressLabel.frame.size.width, cell.addressLabel.frame.size.height);
        //            cell.callButton.frame=CGRectMake(cell.callButton.frame.origin.x, 38, cell.callButton.frame.size.width, cell.callButton.frame.size.height);
        //        }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    CGRect frame = CGRectMake(0, 8, tableView.frame.size.width, 50);
    UIView *tempView = [[UIView alloc] initWithFrame:frame];
    if (canAddEntry)
        tempView.backgroundColor =[UIColor whiteColor];
    else
        tempView.backgroundColor = [UIColor colorWithRed:(239.0f/255.0f) green:(239.0f/255.0f) blue:(244.0f/255.0f) alpha:1];//[UIColor whiteColor];
    UILabel *tempLabel;
    if (!canAddEntry)
        tempLabel= [[UILabel alloc] initWithFrame:CGRectMake(10, 8,tableView.frame.size.width-120, 40)];
    else
        tempLabel= [[UILabel alloc] initWithFrame:frame];
//    if (!tableView.isEditing)
//    {
//        tempLabel.textColor = [UIColor colorWithRed:(100.0f/255.0f) green:(100.0f/255.0f) blue:(100.0f/255.0f) alpha:1];
//        tempLabel.backgroundColor =[UIColor colorWithRed:(239.0f/255.0f) green:(239.0f/255.0f) blue:(244.0f/255.0f) alpha:1];
//        tempLabel.text = @"Phone Numbers";
//        tempLabel.font = [UIFont fontWithName:@"SourceSansPro-Bold" size:14];
//        tempLabel.textAlignment = NSTextAlignmentLeft;
//    }
//    else
//    {
//        tempLabel.textColor = OxfordBlueColor;
//        tempLabel.text = text.uppercaseString;
//        tempLabel.backgroundColor = [UIColor whiteColor];
//        tempLabel.font = [UIFont fontWithName:@"SourceSansPro-Regular" size:14];
...
//    }
    ...
    //    /*-----------------------Another-----------------------*/
    //
    //    UILabel *tempLabel2 = [[UILabel alloc] initWithFrame:CGRectMake(tableView.frame.size.width-120, 8,120, 40)];
    //    if (!tableView.isEditing)
    //        tempLabel2.backgroundColor = [UIColor colorWithRed:(239.0f/255.0f) green:(239.0f/255.0f) blue:(244.0f/255.0f) alpha:1];
    //    else
    //        tempLabel2.backgroundColor = [UIColor whiteColor];
    //        tempLabel2.textColor = [UIColor colorWithRed:(100.0f/255.0f) green:(100.0f/255.0f) blue:(100.0f/255.0f) alpha:1];//kColorHeader;
    //        tempLabel2.numberOfLines=2;
    //        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"Raza Out\n(Using Minutes)"];
    //       // [str addAttribute:NSForegroundColorAttributeName value:[UIColor yellowColor] range:NSMakeRange(10,7)];
    //       // [str addAttribute:NSForegroundColorAttributeName value:[UIColor greenColor] range:NSMakeRange(10,7)];
    //         [str addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"TrebuchetMS-Bold" size:14] range:NSMakeRange(0, 10)];
    //        [str addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"TrebuchetMS" size:8] range:NSMakeRange(9, 15)];
    //        tempLabel2.attributedText = str;
    //       // tempLabel2.attributedText =[self string]; //[NSString stringWithFormat:@"Raza Out\n(Using Minuts)"];
    //    tempLabel2.textAlignment = NSTextAlignmentCenter;
    //    //tempLabel2.font = [UIFont systemFontOfSize:14];
    //    tempLabel2.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    ...
    if (canAddEntry) {
        ...
        NSMutableAttributedString *commentString = [[NSMutableAttributedString alloc] initWithString:@"Add another"];
        [commentString addAttribute:NSForegroundColorAttributeName
                       value:[UIColor colorWithRed:238/255.0 green:55/255.0 blue:89/255.0  alpha:1]
                       range:NSMakeRange(0, [commentString length])];
        [commentString addAttribute:NSFontAttributeName
                              value:[UIFont fontWithName:@"SourceSansPro-Regular" size:18]
                              range:NSMakeRange(0, [commentString length])];
        [commentString addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStyleSingle] range:NSMakeRange(0, [commentString length])];
        
        [tempAddButton setAttributedTitle:commentString forState:UIControlStateNormal];
        ...
        tempAddButton.titleLabel.textAlignment=NSTextAlignmentCenter;
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    UILabel *tempLabel;
    if (!canAddEntry)
        tempLabel= [[UILabel alloc] initWithFrame:CGRectMake(10, 0,tableView.frame.size.width-120, 40)];
    else
        tempLabel= [[UILabel alloc] initWithFrame:frame];
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if (!tableView.isEditing){
        ...
        else if (section == ContactSections_FirstName ) {
            return 0;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (!self.tableView.isEditing) {
        ...
        if (_contact.phoneNumbers.count>(long)indexPath.row) {
           
            UIContactDetailsCell *cell = [tableView cellForRowAtIndexPath:indexPath];
            if (_contact.phoneNumbers.count==1) {
                cell.contentView.backgroundColor=cell.defaultView.backgroundColor =  [UIColor whiteColor];
            }else{
                cell.contentView.backgroundColor=cell.defaultView.backgroundColor =  [UIColor colorWithRed:247.0/255.0f green:247.0/255.0f blue:247.0/255.0f alpha:1.0];
            }
            _phone=_contact.phoneNumbers[indexPath.row];
//            NSString *phone=_contact.phoneNumbers[indexPath.row];
//            ContactDetailsView *view = VIEW(ContactDetailsView);
//            //makeacallparamContactdetail
//            [view makeacallparamContactdetail:phone];
        }
        else
            [RAZA_APPDELEGATE showAlertWithMessage:@"No contact found!" withTitle:@"" withCancelTitle:ERROR_OK];
        //_addressLabel.text
    }
}
