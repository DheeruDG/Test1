        ...
        case LinphoneCallEnd: {
            ...
            if (calls == NULL) {
                while ((currentView == CallView.compositeViewDescription) ||
                    ...
                    if ([[contactarray objectForKey:@"MODE"] isEqualToString:@"OUTGOING"]&&[[contactarray objectForKey:@"MODECALL"] isEqualToString:@"OUTGOINGRECIVED"]) {
                        // RazaLoginViewController *view = VIEW(RazaLoginViewController);
                        //DialerView *view = VIEW(DialerView);
                        //  [PhoneMainView.instance changeCurrentView:view.compositeViewDescription];
                        NSDictionary *dict=contactarray;
                       // [Razauser SharedInstance].ratingDetailDic=dict;
                        [self performSelector:@selector(addratescreen:) withObject:dict afterDelay:0.0 ];
...
-(void)addratescreen:(NSDictionary*)obj
{
...
- (void)startUp {
    @try {
        ...
        if (linphone_core_get_global_state(LC) != LinphoneGlobalOn) {
            ...
            //            [Razauser SharedInstance].modeofview=@"login";
            //            SplashVC *view = VIEW(SplashVC);
            //            [PhoneMainView.instance changeCurrentView:view.compositeViewDescription];
...
- (void)updateApplicationBadgeNumber {
    ...
    int chat=0;
    int recentcounter=0;
    recentcounter+=linphone_core_get_missed_calls_count(LC);
    count += linphone_core_get_missed_calls_count(LC);
    count += [LinphoneManager unreadMessageCount];
    chat+=[LinphoneManager unreadMessageCount];
    count += linphone_core_get_calls_nb(LC);
    [[NSNotificationCenter defaultCenter] postNotificationName:@"messegedatecounter" object:[NSString stringWithFormat:@"%d",chat]];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"missedcallcounter" object:[NSString stringWithFormat:@"%d",recentcounter]];
...
- (void)displayCallError:(LinphoneCall *)call message:(NSString *)message {
    ...
    NSString *lMessage;
    NSString *lTitle;
    if ([lUserName isEqualToString:RAZAPUSHCALLER])
        lUserName=@"This Number";
    if (lUserName.length>=10) {
        
        
        LinphoneAddress *addr = [LinphoneUtils normalizeSipOrPhoneAddress:lUserName];
        UILabel *lbl=[[UILabel alloc]init];
        [ContactDisplay setDisplayNameLabelRaza:lbl forAddress:addr];
        lUserName=lbl.text;
    }
    //   NSLog(@"%@",str);
    ...
    switch (linphone_call_get_reason(call)) {
        ...
        default:
            if (message != nil) {
                ...
                //“ call not answered” please try again later
                //lMessage =[NSString stringWithFormat:NSLocalizedString(@"%@\n: %@", nil), lMessage, @"We are not able to connect. Please try again later."];// [NSString stringWithFormat:NSLocalizedString(@"%@\nReason was: %@", nil), lMessage, message];
                lMessage =[NSString stringWithFormat:NSLocalizedString(@"%@\n: %@", nil), lMessage, @"call not answered please try again later"];
            }
            break;
    }
