  ...
  //  if (([UIApplication sharedApplication].applicationState != UIApplicationStateBackground)) {
	...
	if (_person) {
		...
		if (!_friend) {
          ...
          // linphone_core_get_default_friend_list(LC)
              //  linphone_friend_list_add_friend(linphone_core_get_default_friend_list(LC), _friend);
			}
		}
...
- (NSString *)displayName {
	...
	//	NSString *compositeName = CFBridgingRelease(ABRecordCopyCompositeName(_person));
        ...
        if (lFirstName.length||lLastName.length) {
            compositeName= CFBridgingRelease(ABRecordCopyCompositeName(_person));
        }
