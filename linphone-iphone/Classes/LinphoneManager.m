...
- (void)migrationLinphoneSettings {
	...
	if ([self lpConfigBoolForKey:@"file_transfer_migration_done"] == FALSE) {
		const char *newURL = "http://razaimages.razatelecom.com/raza/raza_test.php";//"http://54.215.183.119/floovr/raza/raza_test.php";//"https://www.linphone.org:444/lft.php";
...
//                            
//                        }
//                        data->notification.soundName = RAZACALLINGTONE;//ring;
//					} else {
//                        data->notification.soundName = RAZACALLINGTONE;//@"shortring.caf";
...
//    // [NSNotificationCenter.defaultCenter postNotificationName:@"callUpdateEventforpausenotif" object:self userInfo:dict];
//    
//}
...
#pragma deploymate pop
    ...
    [ContactDisplay setDisplayNameLabelRaza:nameLbl forAddress:addr];
    address=nameLbl.text;
    if (state == LinphoneCallIncomingReceived) {
...
//        video=    linphone_call_params_video_enabled(linphone_call_get_remote_params(call));
//            video2=    linphone_call_params_video_enabled(linphone_call_get_current_params(call));
                        video = (([UIApplication sharedApplication].applicationState != UIApplicationStateBackground) &&
                                 linphone_core_get_video_policy(LC)->automatically_accept &&
                                 linphone_call_params_video_enabled(linphone_call_get_remote_params(call)));
//            if (video2==TRUE&&video==TRUE)
//                   [LinphoneManager.instance.providerDelegate reportIncomingCallwithUUID:uuid handle:address video:video];
//            
//            else
//               [LinphoneManager.instance.providerDelegate reportIncomingCallwithUUID:uuid handle:address video:NO];
           [LinphoneManager.instance.providerDelegate reportIncomingCallwithUUID:uuid handle:address video:video];
#else
            ...
            content.title = NSLocalizedString(@"Incoming call", nil);
            content.body = address;
            content.sound = [UNNotificationSound soundNamed:@"Ringtone2.mp3"];//notes_of_the_optimistic.caf
            content.categoryIdentifier = @"call_cat";
            content.userInfo = @{ @"CallId" : callId };
            UNNotificationRequest *req =
            [UNNotificationRequest requestWithIdentifier:@"call_request" content:content trigger:NULL];
            ...
            if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_9_x_Max) {
                ...
                if (data->notification) {
                    ...
                    if ([[UIDevice currentDevice].systemVersion floatValue] >= 8 &&
                        ...
                        NSString *ring =
                        ([LinphoneManager bundleFile:[self lpConfigStringForKey:@"local_ring" inSection:@"sound"].lastPathComponent]
                         ?: [LinphoneManager bundleFile:@"Ringtone2.mp3"])//notes_of_the_optimistic.caf
                        .lastPathComponent;
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_9_x_Max) {
            ...
            if (uuid) {
                ...
                [self.providerDelegate.calls removeObjectForKey:uuid];
                    ...
                    NSLog(@"Im on the main thread for call %@",uuid);
...
static void linphone_iphone_popup_password_request(LinphoneCore *lc, const char *realmC, const char *usernameC,
	...
	if ([PhoneMainView.instance currentView] != AssistantView.compositeViewDescription) {
       ...
       // [RAZA_APPDELEGATE showAlertWithMessage:@"call not connected!" withTitle:nil withCancelTitle:@"ok"];
...
#pragma deploymate pop
   ...
   // NSString *from;// = [FastAddressBook displayNameForAddress:remoteAddress]; this was for normal contact
    UILabel *label=[[UILabel alloc]init];
    
    Contact *contact = [FastAddressBook getContactWithAddressRaza:remoteAddress];
    if (contact) {
        [ContactDisplay setDisplayNameLabelRaza:label forContact:contact];
    } else {
        label.text = [FastAddressBook displayNameForAddress:remoteAddress];
    }
    from=label.text;
    
    [ContactDisplay setDisplayNameLabelRaza:label forAddress:remoteAddress];
    /*--this changes happen for contact-----*/
...
#pragma deploymate pop
    ...
    // Post event
//    NSString *from1 =@"9900000099";// [notification.userInfo objectForKey:@"from_addr"];
...
//     //  int unreadMessage = [LinphoneManager unreadMessageCount];
//     
//     NSString *abc = @"hellooo-saurabh";
//     
//    // room = (__bridge LinphoneChatRoom *)([NSValue valueWithPointer:room]);
//     LinphoneChatMessage *msg = linphone_chat_room_create_message(room, [abc UTF8String]);
//    // LinphoneAddress *addr = [LinphoneUtils normalizeSipOrPhoneAddress:@"99"];
//    //char *uri = linphone_address_as_string_uri_only(addr);
//    const char *cfilename=[@"1112223333" UTF8String];
//    linphone_chat_message_add_custom_header(msg, cfilename, "Call-ID");
//    [(__bridge LinphoneManager *)linphone_core_get_user_data(lc) onMessageReceived:lc room:room message:msg];
    
     /*
     
     //  message = (__bridge LinphoneChatMessage *)([NSValue valueWithPointer:msg]);
	...
	if (theLinphoneCore != nil) {
		...
		if ((flags == 0) || (flags & networkDownFlags)) {
			...
			if (lm.connectivity != newConnectivity) {
                ...
                if (newConnectivity==wifi)
                    checkfor_availablenetwork=@"1";
                else if (newConnectivity==wwan)
...
- (void)finishCoreConfiguration {
    ...
    NSString *path = [LinphoneManager bundleFile:@"nowebcamCIF.jpeg"];//@"nowebcamCIF.jpg"];
	if (path) {
...
- (void)createLinphoneCore {
	...
	NSString *ring =
		([LinphoneManager bundleFile:[self lpConfigStringForKey:@"local_ring" inSection:@"sound"].lastPathComponent]
			 ?: [LinphoneManager bundleFile:@"Ringtone2.mp3"])
			.lastPathComponent;////notes_of_the_optimistic.caf
	NSString *ringback =
	...
	NSString *hold =
		([LinphoneManager bundleFile:[self lpConfigStringForKey:@"hold_music" inSection:@"sound"].lastPathComponent]
			 ?: [LinphoneManager bundleFile:@"hold.mkv"])//hold.mkv
			.lastPathComponent;
...
- (void)destroyLinphoneCore:(NSString*)mode {
	...
	if (proxyCfg) {
		...
		if ([LinphoneManager.instance lpConfigBoolForKey:@"backgroundmode_preference"]) {
			...
			if ([[UIApplication sharedApplication]
					setKeepAliveTimeout:0 /*(NSTimeInterval)linphone_proxy_config_get_expires(proxyCfg)*///600
								handler:^{
...
- (void)audioRouteChangeListenerCallback:(NSNotification *)notif {
    ...
    [self customchangebacktospeaker:notif];
...
-(void)customchangebacktospeaker:(NSNotification *)notif
{
...
//        
// [[Razauser SharedInstance]ShowWaiting:RAZAPUSHVIDEOCALLMESSAGE];
//  
//    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
//        [self load:lcallParams andtrueval:video andcall:call];
//    });
//    }
//    else
//    {
...
-(void)load:(LinphoneCallParams*)lcallParams andtrueval:(BOOL)video andcall:(LinphoneCall *)call
{
     linphone_call_params_enable_video(lcallParams, video);
...
- (BOOL)call:(const LinphoneAddress *)iaddr andmodeVideoAudio:(BOOL)modeVideoAudio{
...
//    
////TODO:Umenit(video codec)
//        linphone_call_params_enable_video(lcallParams, modeVideoAudio);
// 
...
//	return TRUE;
    ...
    MODECALLAUDIOVIDEO=modeVideoAudio;
    /*-------------Callkit Impliment-------------*/
    if (!linphone_core_is_network_reachable(theLinphoneCore)) {
        UIAlertController *errView = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Network Error", nil)
                                                                         message:NSLocalizedString(@"There is no network connection available, enable WIFI or WWAN prior to place a call", nil)
    ...
    if (linphone_core_get_calls_nb(theLinphoneCore) < 1 &&
        ...
        [self doCall:iaddr andmodevideoaudio:MODECALLAUDIOVIDEO];
    }
    return TRUE;
...
- (BOOL)doCall:(const LinphoneAddress *)iaddr andmodevideoaudio:(BOOL)modevideoaudio
{
    LinphoneAddress *addr = linphone_address_clone(iaddr);
    NSString *displayName = [FastAddressBook displayNameForAddress:addr];
    ...
    if (LinphoneManager.instance.nextCallIsTransfer) {
        ...
        if (call) {
            ...
            if (data == nil) {
                ...
                data->videoRequested = linphone_call_params_video_enabled(lcallParams);
                BOOL vv=linphone_call_params_video_enabled(lcallParams);
                if (vv) {
