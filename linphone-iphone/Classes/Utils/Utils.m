...
+ (UIImage *)selfAvatar {
	...
	if (!ret) {
		ret = [UIImage imageNamed:@"avatar_male.png"];//@"avatar_female.png"
	}
	return ret;
}
  ...
  //  NSString *formatstrformsg;
	...
	switch (format) {
		...
		case LinphoneDateHistoryDetails:
			formatstr = NSLocalizedString(@"hh:mma", @"Date formatting in History Details (also see "
																	  @"http://cybersam.com/ios-dev/"
																	  @"quick-guide-to-ios-dateformatting)");
            //EEE dd MMM 'at' hh:mm a
			break;
		...
		case LinphoneDateChatBubble:
			if (sameDay) {
//                formatstrformsg=@"MM/dd yyyy";
//                NSDateFormatter *dateFormatter2 = [[NSDateFormatter alloc] init];
//                [dateFormatter2 setDateFormat:formatstrformsg];
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"messegedate" object:[dateFormatter2 stringFromDate:messageDate]];
    ...
    //[[NSNotificationCenter defaultCenter] postNotificationName:@"messegedate" object:[NSString stringWithFormat:@"%ld %ld %ld",(long)dateComponents.month,(long)dateComponents.year,(long)dateComponents.day]];
    if (format==LinphoneDateChatBubble&&(sameDay==TRUE)) {
        return [NSString stringWithFormat:@"%@ Today",[dateFormatter stringFromDate:messageDate]];
    }
    else
...
+ (void)setDisplayNameLabelRaza:(UILabel *)label forContact:(Contact *)contact {
    label.text = [FastAddressBook displayNameForContact:contact];
#if 0
    NSString *lLastName = CFBridgingRelease(ABRecordCopyValue(contact, kABPersonLastNameProperty));
    NSString *lLocalizedLastName = [FastAddressBook localizedLabel:lLastName];
...
+ (BOOL)setRazaUser:(Contact *)contact {
    BOOL yesfound=NO;
    ...
    }         //[[[Razauser SharedInstance]getRazauser] containsObject:contact.phoneNumbers]
...
+ (void)setDisplayNameLabelRaza:(UILabel *)label forAddress:(const LinphoneAddress *)addr {
