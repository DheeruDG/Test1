...
- (void)transformCodecsToKeys:(const MSList *)codecs {
	...
	for (; elem != NULL; elem = elem->next) {
		...
		if (pref) {
            ...
            if ([pref isEqualToString:@"g729_preference"]) {
              
                linphone_core_enable_payload_type(LC,pt,true);
                [self setBool:true forKey:pref];
            }
			else
...
//		[self setBool:YES forKey:@"accept_video_preference"];//(pol->automatically_accept)
...
//        [self setBool:linphone_core_self_view_enabled(LC) forKey:@"self_video_preference"];
        ...
        [self setBool:YES forKey:@"start_video_preference"];
        [self setBool:YES forKey:@"accept_video_preference"];
        [self setBool:YES forKey:@"self_video_preference"];
        BOOL previewEnabled = [lm lpConfigBoolForKey:@"preview_preference" withDefault:YES];
        [self setBool:IPAD && previewEnabled forKey:@"preview_preference"];
